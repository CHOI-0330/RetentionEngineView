/**
 * POST /api/avatar/generate - アバター画像生成（バックエンドプロキシ）
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${BACKEND_URL}/avatar/generate`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.message || "Failed to start avatar generation" },
