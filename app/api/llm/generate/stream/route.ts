...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[llm-generate-stream][POST]", {
      conversationId,
    ...
    if (!backendResponse.ok) {
      ...
      console.error("[llm-generate-stream][error]", backendResponse.status, text);
      return new Response(
        JSON.stringify({
    ...
    console.error("[llm-generate-stream][POST][error]", error);
    const message =
