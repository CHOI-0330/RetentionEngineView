...
import type { MbtiType } from "../../../src/domain/mbti.types";
...
type GetMbtiResponseDto = {
  mbti: MbtiType | null;
};

type UpdateMbtiResponseDto = {
  message?: string;
};
...
const getMbti = (userId: string, accessToken: string) =>
  callBackend<GetMbtiResponseDto>(
    `/users/mbti?userId=${encodeURIComponent(userId)}`,
    { method: "GET" },
    accessToken
  );

const updateMbti = (input: { mbti: MbtiType; userId: string }, accessToken: string) =>
  callBackend<UpdateMbtiResponseDto>(
    "/users/mbti",
    {
      method: "PUT",
      body: JSON.stringify({
...
export async function PUT(request: NextRequest) {
  try {
    const { accessToken, user } = await requireAuth(request);
    const body = (await request.json()) as { mbti?: MbtiType; userId?: string };
    const mbti = body.mbti;
    if (!mbti) {
      throw new HttpError(400, "mbti is required.");
    }
    const targetUserId = body.userId ?? user.userId;
    if (targetUserId !== user.userId) {
      throw new HttpError(403, "Forbidden");
    }
    await updateMbti({ mbti, userId: targetUserId }, accessToken);
