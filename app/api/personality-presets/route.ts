...
type PersonalityPresetDto = {
  id: string;
  displayName: string;
};

type GetPresetsResponseDto = {
...
const getPresets = (accessToken: string) =>
  callBackend<GetPresetsResponseDto>(
    "/personality-presets",
    { method: "GET" },
    accessToken
  );
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await getPresets(accessToken);
    return NextResponse.json({ data: { presets: response?.presets ?? [] } });
  } catch (error) {
