...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.originalAiMessage || !Array.isArray(body.chatHistory)) {
      return new Response(
        JSON.stringify({
          error: "originalAiMessage and chatHistory are required.",
    ...
    const backendResponse = await fetch(
      `${BACKEND_BASE_URL}/question-cards/generate/stream`,
      {
        method: "POST",
        headers: {
    ...
    if (!backendResponse.ok) {
      ...
      console.error(
        "[question-cards-generate-stream][error]",
    ...
    console.error("[question-cards-generate-stream][POST][error]", error);
    const message =
