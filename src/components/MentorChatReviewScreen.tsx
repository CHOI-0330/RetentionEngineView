...
export function MentorChatReviewScreen({ studentId, onBack }: MentorChatReviewScreenProps) {
  ...
  const getMessagesForStudent = (id: string): Message[] => {
    const messageSets = {
      '1': [
        {
          id: '1',
          content: "こんにちは！二次方程式の理解に苦労しています。実際の問題で使う方法を教えてもらえますか？",
  ...
  const [messages, setMessages] = useState<Message[]>(getMessagesForStudent(studentId));
  ...
  const handleFeedback = (messageId: string, isGood: boolean) => {
    console.log(`Feedback for message ${messageId}: ${isGood ? 'Good' : 'Bad'}`);
    // ここで実際のフィードバック処理を実装
  };
      ...
      <div className="border-b bg-white px-4 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">添削・レビューモード</p>
          </div>
        </div>
      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {message.sender === 'ai' && message.canEdit && (
                          <div className="flex items-center gap-2 pt-2 border-t border-border/20">
                            <span className="text-xs font-medium">品質評価:</span>
                            <Button
                              size="sm"
                              variant="outline"
